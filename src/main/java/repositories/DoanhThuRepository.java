...
//        }
    ...
    public List<Object[]> getListtop3() {
        ...
        try {
            Session session = HibernatUtil.getFACTORY().openSession();
            Transaction transaction = session.getTransaction();
            Query q = session.createQuery("select B.Id,B.TenSP,B.SoLuong ,Sum(A.SoLuong),SUM(A.DonGia) from HoaDonChiTiet A , ChiTietDoGo B , HoaDon C\n"
    ...
    public int getListNgungKinhdoanh() {
        try {
            ...
            Session session = HibernatUtil.getFACTORY().openSession();
            Transaction transaction = session.getTransaction();
            Query q = session.createQuery("select COUNT(A.SoLuong),COUNT(A.SoLuong) from ChiTietDoGo A where A.TrangThai = 0");
            List<Object[]> list = q.getResultList();
            for (Object[] o : list) {
                a = Integer.parseInt(o[0].toString());
            }
    ...
    public List<Object[]> getListNhanVien() {
        ...
        try {
            Session session = HibernatUtil.getFACTORY().openSession();
            Transaction transaction = session.getTransaction();
            Query q = session.createQuery("select A.Id,A.Ma,A.HoTen,C.TenChucVu,COUNT(B.Id) from NhanVien A , HoaDon B ,ChucVu C\n"
...
//        }
    ...
    public int getDoanhthuTheoNam() {
        try {
            ...
            Session session = HibernatUtil.getFACTORY().openSession();
            Transaction transaction = session.getTransaction();
            Query q = session.createQuery("select sum(ThanhTien),sum(ThanhTien) from HoaDon \n"
    ...
    public int getsotiennhaptheonam() {
        try {
            ...
            Session session = HibernatUtil.getFACTORY().openSession();
            Transaction transaction = session.getTransaction();
            Query q = session.createQuery("select Sum(TongTienNhap),Sum(TongTienNhap) from LichSuNhap\n"
    ...
    public List<Object[]> getListHoaDonDaChon(String nbd, String nkt) {
        ...
        try {
            Session session = HibernatUtil.getFACTORY().openSession();
            Query q = session.createQuery("select Id,Ma,NgayThanhToan,Sum(ThanhTien) from  HoaDon \n"
